...
class _ImportantMessagesPageState extends State<ImportantMessagesPage> {
  ...
  Future<DocumentSnapshot<Map<String, dynamic>>?> getLatestQuiz() async {
    try {
      final QuerySnapshot<Map<String, dynamic>> latestQuizQuerySnapshot =
          await widget.chatPageState.chatDocRef
              .collection('quiz')
  ...
  bool isUserInPasserList(DocumentSnapshot<Map<String, dynamic>> quiz) {
    ...
    final String currentUserId = widget.chatPageState.currentUser.uid;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: StreamBuilder<QuerySnapshot>(
        stream: widget.chatPageState.chatDocRef
            .collection('imp_msg')
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return IgnorePointer(
                ignoring: widget.chatPageState.chat.bEndProject,
                child: Card(
      ...
      floatingActionButton:
          isBtnEnable && !widget.chatPageState.chat.bEndProject
              ? FloatingActionButton(
                  onPressed: () {
                    Navigator.push(
                      context,
...
class SimpleImportantMessage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: StreamBuilder<QuerySnapshot>(
        stream: chatPageState.chatDocRef
            .collection('imp_msg')
            .orderBy('timeStamp', descending: true) // 최근 시간 순으로 정렬
            .snapshots(),
        builder: (context, snapshot) {
