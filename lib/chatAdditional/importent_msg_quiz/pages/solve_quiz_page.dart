...
class solve_quizState extends State<solve_quiz> {
  ...
  Future<void> setQuiz(Timestamp quiz_C_date, List<String> quiz_passer) async {
    try {
      await widget.chatPageState.chatDocRef.collection('quiz').doc().set({
    ...
    try {
      ...
      if (documents.isNotEmpty) {
        ...
        if (difference.inHours < 24) {
          ...
          if (!quizPasser.contains(widget.chatPageState.currentUser.uid)) {
            //! 유저가 푼 사람 목록에 없으면
            quizPasser.add(widget.chatPageState.currentUser.uid);
          }
  ...
  Future<List<imp_msg>> getimpMsgList() async {
    QuerySnapshot querySnapshot = await widget.chatPageState.chatDocRef
        .collection('imp_msg')
...
class BuildOrderQuiz extends StatefulWidget {
  ...
  final ChatPageState chatDataParent;
